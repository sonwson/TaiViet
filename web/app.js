import {createAdminWorkspace} from './admin-workspace.mjs';
...
const state={session:null,sentence:null,review:null,sentenceOffset:0,reviewOffset:0,dictOffset:0,adminOffset:0,transFilters:{status:'approved',source:'all',meaning:'all',roman:'all'},reviewFilters:{status:'approved',sentence_status:'all',source:'all',meaning:'has_meaning',roman:'all',validation_status:'all'}};
...
async function loadReview(){
  const params=new URLSearchParams({
    ...
    validation_status: state.reviewFilters.validation_status,
...
$('review-filter-apply')?.addEventListener('click',()=>{
  state.reviewFilters={
    ...
    validation_status: $('review-filter-validation')?.value || 'all',
...
$('admin-login').addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget;try{const f=form.elements;const payload=f.token?{token:f.token.value}:{username:f.username?.value,password:f.password?.value};const res=await api('admin/login',payload);if(res.token)localStorage.setItem('tai_admin_token',res.token);form.reset();await refreshSession();await loadAdmin();notice('Đăng nhập thành công (JWT)!');}catch(e){notice(e.message,true);}});
$('admin-logout').addEventListener('click',async()=>{try{localStorage.removeItem('tai_admin_token');await api('admin/logout',{});await refreshSession();adminWorkspace.reset();$('admin-results').replaceChildren();}catch(e){notice(e.message,true);}});
...
const adminWorkspace=createAdminWorkspace({api,node,notice,attachAutoMatch,loadStats:loadAdminStats});
async function loadAdmin(){await adminWorkspace.load(true);}
