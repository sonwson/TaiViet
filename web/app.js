...
const state={session:null,sentence:null,review:null,sentenceOffset:0,reviewOffset:0,dictOffset:0,adminOffset:0,transFilters:{status:'approved',source:'all',meaning:'all',roman:'all'},reviewFilters:{status:'approved',sentence_status:'all',source:'all',meaning:'has_meaning',roman:'all'}};
...
function sentenceCard(prompt,row,review=false){
  ...
  const taiLabel=node('label','Câu chữ Tai (bắt buộc):');
  ...
  const meanLabel=node('label','Nghĩa tiếng Việt (bắt buộc):');
  ...
  const btnSubmitCorr=node('button', isAdmin ? 'Lưu sửa câu gốc (Admin - Ghi đè áp dụng)' : 'Gửi đề xuất sửa câu gốc (Admin duyệt)');
  btnSubmitCorr.type='button';
  btnSubmitCorr.addEventListener('click', async()=>{
    const tVal=taiInput.value.trim();
    const rVal=romInput.value.trim();
    const mVal=meanInput.value.trim();
    if(!tVal || !rVal || !mVal){
      notice('Vui lòng điền đầy đủ cả 3 trường: Câu chữ Tai, Phiên âm và Nghĩa tiếng Việt.', true);
      return;
    }
    ...
    try {
      const res = await api('sentence-corrections', {
        ...
        suggested_tai_text: tVal,
        suggested_romanization: rVal,
        suggested_meaning: mVal
      });
  ...
  if(review){
    if(row.vietnamese_text){
      prompt.append(node('p','Bản dịch cần kiểm tra','review-label'),node('p',row.vietnamese_text));
      $('review-form').hidden=false;
    }else{
      prompt.append(node('p','Bản dịch','review-label'),node('p','Câu này chưa có bản dịch. Bạn có thể mở mục sửa câu gốc phía trên để bổ sung bản dịch/nghĩa.','hint'));
    ...
    for(const t of translations){
      const tP=node('p',t.vietnamese_text);
      if(t.status==='pending')tP.append(node('small',' (Chờ duyệt)','hint'));
      prompt.append(tP);
    }
  }
}
...
$('trans-filter-apply')?.addEventListener('click',()=>{
  state.transFilters={
    status: $('trans-filter-status')?.value || 'approved',
    source: $('trans-filter-source')?.value || 'all',
    meaning: $('trans-filter-meaning')?.value || 'all',
    roman: $('trans-filter-roman')?.value || 'all'
  };
  state.sentenceOffset=0;
  loadSentence().catch(e=>notice(e.message,true));
});
wireForm('translation-form','translations',()=>({sentence_id:state.sentence?.id}),async ()=>{
  state.sentenceOffset++;
  await loadSentence();
});
wireForm('naturalness-form','sentence-reviews',()=>({sentence_id:state.sentence?.id}),async ()=>{
  state.sentenceOffset++;
  await loadSentence();
});
async function loadReview(){
  const params=new URLSearchParams({
    ...
    sentence_status: state.reviewFilters.sentence_status,
    source: state.reviewFilters.source,
    meaning: state.reviewFilters.meaning,
  ...
  if(state.review)sentenceCard($('review-prompt'),state.review,true);
    ...
    $('review-prompt').textContent='Không có bản ghi phù hợp với bộ lọc này.';
    $('review-form').hidden=true;
  }
}
...
$('review-filter-apply')?.addEventListener('click',()=>{
  state.reviewFilters={
    sentence_status: $('review-filter-sentence-status')?.value || 'all',
    status: $('review-filter-status')?.value || 'approved',
    source: $('review-filter-source')?.value || 'all',
    meaning: $('review-filter-meaning')?.value || 'has_meaning',
    roman: $('review-filter-roman')?.value || 'all'
  };
...
$('admin-logout').addEventListener('click',async()=>{try{localStorage.removeItem('tai_admin_token');await api('admin/logout',{});await refreshSession();adminRequest++;adminSelection.reset();$('admin-results').replaceChildren();}catch(e){notice(e.message,true);}});
...
for(const id of ['admin-table','admin-status'])$(id).addEventListener('change',()=>{state.adminOffset=0;loadAdmin().catch(e=>notice(e.message,true));});
$('admin-query').addEventListener('input',()=>{adminRequest++;adminSelection.reset();$('admin-results').replaceChildren();$('admin-more').disabled=true;});
$('admin-query').addEventListener('keydown',e=>{if(e.key==='Enter')$('admin-load').click();});
